...
class StageNet(nn.Module):
    ...
    def forward(self, input, time, device):
        ...
        for t in range(time_step):
            ...
            # print('local_h pre replace conv shape: ', local_h.shape)
            local_h = self.nn_conv(local_h)
            # local_h = self.replace_conv(local_h)
            # print('local_h after replace conv shape: ', local_h.shape)
            local_h = local_h.squeeze(-1)
            # print('local_h final shape: ', local_h.shape)
