...
def parse_arguments(parser):
    ...
    parser.add_argument('--continueTrain', type=int, default=0, help='continue to train')
...
if __name__ == '__main__':
    ...
    if args.test_mode == 1:
        ...
        file_name = './drive/My Drive/StageNet/saved_weights/'+args.file_name
        ...
        if args.continueTrain:
          checkpoint = torch.load(file_name)
          start_epoch = int(checkpoint['chunk'])
          print("last saved model is in chunk {}, continue to train".format(start_epoch))
          model.load_state_dict(checkpoint['net'])
          optimizer.load_state_dict(checkpoint['optimizer'])
          max_auprc = float(checkpoint['max_auprc'])
        ...
        val_loss = []
        batch_loss = []
        print('max_auprc = ', max_auprc)
        print('starting epoch = ', start_epoch+1)
        
        
        for each_chunk in range(start_epoch+1, args.epochs):
            cur_batch_loss = []
            model.train()
            ...
            with torch.no_grad():
                ...
                state = {
                        ...
                        'chunk': str(each_chunk),
                        'max_auprc': str(max_auprc)
                    }
                torch.save(state, file_name + '_record_epoch' + str(each_chunk))
                print('\n------------ Save model ------------\n')
                if cur_auprc > max_auprc:
                    max_auprc = cur_auprc
                    torch.save(state, file_name + '_epoch' + str(each_chunk))
                    print('\n------------ Save best model ------------\n')
        ...
        checkpoint = torch.load(file_name)
        save_chunk = int(checkpoint['chunk'])
        print("last saved model is in chunk {}".format(save_chunk))
        model.load_state_dict(checkpoint['net'])
        optimizer.load_state_dict(checkpoint['optimizer'])
        model.eval()
